...
def load_data(
    root_path, rgb_path, depth_path, camera_id, depth_threshold=2.0, visualize=True
):
    ...
    pcd, pcd_np, color_filtered = depth_to_pointcloud(
        rgb, depth, intrinsics, depth_threshold
    )
    if visualize:
        to_vis = []
        to_vis.append(pcd)
...
def load_obj_as_pointcloud(obj_file_path, sample_points=0):
    ...
    print(
        f"Mesh loading completed: {len(mesh.vertices)} vertices, {len(mesh.triangles)} triangles"
    )
...
def draw_charuco_origin(
    image, charuco_dict, intrinsics_matrix, dist_coeffs, board, axis_length=0.05
):
    """
    ...
    if charuco_ids is not None and len(charuco_corners) > 3:
        ...
        if valid:
            ...
            image_points, _ = cv2.projectPoints(
                axis_points, rvec, tvec, intrinsics_matrix, dist_coeffs
            )
            ...
            image = cv2.line(
                image, tuple(image_points[0]), tuple(image_points[1]), (0, 0, 255), 2
            )  # X-axis (red)
            image = cv2.line(
                image, tuple(image_points[0]), tuple(image_points[2]), (0, 255, 0), 2
            )  # Y-axis (green)
            image = cv2.line(
                image, tuple(image_points[0]), tuple(image_points[3]), (255, 0, 0), 2
            )  # Z-axis (blue)

            # Draw the origin point
            image = cv2.circle(
                image, tuple(image_points[0]), 5, (0, 255, 255), -1
            )  # Origin (yellow)
