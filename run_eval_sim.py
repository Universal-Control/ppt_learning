...
def eval_in_one_process(rank, world_size, cfg, domain, queue: JoinableQueue):
    ...
    try:
        ...
        policy = hydra.utils.instantiate(
            cfg.network, max_timestep=cfg.rollout_runner.max_timestep
        )
        ...
        runner = hydra.utils.instantiate(
            cfg.rollout_runner, world_size=world_size, rank=rank
        )
        ...
        model_names = [
            name
            for i, name in enumerate(cfg.train.model_names)
            ...
            if isinstance(result, tuple) and len(result) == 3:
                ...
                if isinstance(extra_info, tuple) and len(extra_info) == 3:
                    ...
                    for key in subtask_success_nums:
                        print(
                            f"Subtask success rate for {key} is: {float(subtask_success_nums[key]) / episode_num}"
                        )
                        subtask_success_sr[key] = (
                            float(subtask_success_nums[key]) / episode_num
                        )
                else:
                    subtask_success_sr = {}
            else:
    ...
    for model_name in model_names:
        ...
        with open(os.path.join(model_dir, f"{cfg.eval_log_name}.txt"), "at") as t:
            ...
            for key in subtask_success_sr:
                t.write(
                    f"Subtask success rate for {key} is: {subtask_success_sr[key]}\n"
                )
...
def run_parallel_eval(cfg, domain):
    ...
    mp.spawn(
        eval_in_one_process,
        args=(world_size, cfg, domain, shared_queue),
        nprocs=world_size,
        join=False,
        daemon=True,
    )
...
def run(cfg):
    ...
    if eval_mode == "auto":
        ...
        use_parallel = (
            n_procs > 1 and n_gpus >= n_procs and len(cfg.train.model_names) > 1
        )
