...
parser.add_argument(
    "--failed_dataset_path",
    default="/data/datasets/ur5_put_bowl_in_microwave_and_close/put_bowl_in_microwave_8_demos_generated_interrupt_simplify_gripper_retry_more_wait_520.zarr",
)
parser.add_argument(
    "--dataset_path",
...
failed_zarr_path = (
    Path(args.failed_dataset_path).parent
    / f"{Path(args.failed_dataset_path).stem}_failed.zarr"
)
...
if args.type == 0:
    ...
    faild_rigid_pos = failed_zarr_root["data"]["initial_state"]["rigid_object"]["bowl"][
        "root_pose"
    ][:, :2]
    faild_articulation_pos = failed_zarr_root["data"]["initial_state"]["articulation"][
        "microwave"
    ]["root_pose"][:, :2]
else:
    last_idx = zarr_root["meta/episode_ends/"][()]
    rigid_pos = zarr_root["data"]["states"]["rigid_object"]["bowl"]["root_pose"][
        last_idx - 1, :2
    ]
    articulation_pos = zarr_root["data"]["states"]["articulation"]["microwave"][
        "root_pose"
    ][last_idx - 1, :2]

    last_failed_idx = failed_zarr_root["meta/episode_ends/"][()]
    faild_rigid_pos = failed_zarr_root["data"]["states"]["rigid_object"]["bowl"][
        "root_pose"
    ][last_failed_idx - 1, :2]
    faild_articulation_pos = failed_zarr_root["data"]["states"]["articulation"][
        "microwave"
    ]["root_pose"][last_failed_idx - 1, :2]
...
rect_width, rect_height = rigid_center[0] + random_x[1] - (
    rigid_center[0] + random_x[0]
), rigid_center[1] + random_y[1] - (rigid_center[1] + random_y[0])
rect = Rectangle(
    (rigid_center[0] + random_x[0], rigid_center[1] + random_y[0]),
    rect_width,
    rect_height,
    linewidth=1,
    edgecolor="g",
...
ax.set_title(
    f"Position Distribution Visualization {'Start' if args.type==0 else 'End'}"
)
